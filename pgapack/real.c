...
/*I****************************************************************************
   PGARealGetString - writes a real-valued string to an double array.  This routine
   casts the void string pointer it is passed as the second argument.

   Inputs:
      ctx - context variable
      da  - a pointer to a double array
      p   - index of the string to write out
      pop - symbolic constant of the population string p is in
   ...
   Example:
      Write string s to the double array

      PGAContext *ctx;
      int s;
      :
     PGARealGetString( ctx, da, s, PGA_NEWPOP );

****************************************************************************I*/
void PGARealGetString (PGAContext *ctx, PGAReal *da, int p, int pop)
{
    PGAReal *c = (PGAReal *)PGAGetIndividual(ctx, p, pop)->chrom;
    int i;

    PGADebugEntered("PGARealGetString");
    ...
    PGADebugExited("PGARealGetString");
}
