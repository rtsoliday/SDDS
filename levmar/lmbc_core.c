                                      ...
                                      */
{
  ...
  LM_REAL t, t0;
  LM_REAL steptl=CNST(1e3)*sqrt(LM_REAL_EPSILON), jacTeDp;
  //LM_REAL tmin=CNST(1e-12);
  LM_REAL tming=CNST(1e-18); /* minimum step length for LS and PG steps */
                                                        ...
                                                        while(1){
                                                          ...
                                                          */
                                                          ...
                                                          if(jacTeDp<=-rho*pow(Dp_L2, _POW_/CNST(2.0))){
                                                            ...
                                                            tmp=sqrt(p_L2); stepmx=1e3*( (tmp>=1.0)? tmp : 1.0 );
                                                          ...
                                                          gradproj: /* Note that this point can also be reached via a goto when LNSRCH() fails */
                                                            ...
                                                            t0=(tmp<=tini)? tmp : tini; /* guard agains poor scaling & large steps; see (3.50) in C.T. Kelley's book */
...
/* following struct & function won't be necessary if a true secant version
 * of LEVMAR_BC_DIF() is implemented...
 */
                                      ...
                                      */
{
  ...
  /* //fprintf(stderr, RCAT("\nWarning: current implementation of ", LEVMAR_BC_DIF) "() does not use a secant approach!\n\n"); */
    ...
    data.hxx=data.hx+n;
    data.adata=adata;
    data.delta=(opts)? fabs(opts[4]) : LM_DIFF_DELTA; /* // no central differences here...*/

      ret=LEVMAR_BC_DER(func, LMBC_DIF_JACF, p, x, m, n, lb, ub, itmax, opts, info, work, covar, (void *)&data);
