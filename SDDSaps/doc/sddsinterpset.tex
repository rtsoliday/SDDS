...
sddsinterpset [input] [output] [-pipe=[input][,output]]
  ...
  \item \textbf{switches:}
    \begin{itemize}
      ...
      \item \verb|-threads=number| --- Specifies the number of referenced data files to read and
        interpolate concurrently for each input page. The default is 1. SDDS output operations are
        performed serially.
    \end{itemize}
  \item \textbf{files:}
