\begin{sddsprog}{sddssampledist}
  ...
  \item \textbf{synopsis:}
    ...
    sddssampledist [input] [output] [-pipe=[in][,out]]
      ...
      [-columns=...] [-samples=integer] [-seed=integer] [-threads=number]
    \end{verbatim}
  \item \textbf{files:}
  ...
  \item \textbf{switches:}
    \begin{itemize}
      ...
      \item \verb|-threads=number| --- Specifies the number of \verb|datafile| distributions to read concurrently. The default is 1. Sampling and SDDS output operations are performed serially, preserving seeded output order.
    \end{itemize}
