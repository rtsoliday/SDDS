...
\item \progref{sddsgroupedenvelope} --- Analyzes pages from multiple input files grouped by a parameter
to find minima, maxima, averages, standard-deviations, etc., on a row-by-row basis. Example application:
finding separate envelopes for many devices without creating intermediate files.
...
\hspace*{5mm}{\tt sddsconvert {\em input} = this text is a comment = -binary {\em output}}\\
