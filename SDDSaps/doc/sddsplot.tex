...
\item Initiating a plot request:\\
  ...
  \item {\tt -define=\{column | parameter\},{\em equation}[,{\em definition-entries}]} --- Defines a column or parameter
    for plotting, in terms of columns and parameters in the input file. This option must be given inside a plot request
    and the request must have only one file.
...
\item Controlling type of plotting:
  ...
  \item {\tt -graphic={\em element}[,type={\em integer}|@{\em columnName}][,fill][,subtype=\{{\em integer} | type | @{\em columnName}\}]}
...
For devices that support color lines, the subtype for lines specifies the choice of dash-dot pattern.
...
If a column is given for the type or subtype, the data is expected to give integer values.
...
(\hyperref[SDDSediting]{SDDS Editing}).
...
labels, the text may be edited using Toolkit editing commands (\hyperref[SDDSediting]{SDDS Editing}).
...
named string parameter.  Any legend text may be editing using SDDS editing commands\hyperref[SDDSediting]{SDDS Editing} via the {\tt editCommand} qualifier.  If {\tt firstFileOnly} is given, only the first file
...
The \href{sddsplot-character-charts/index.html}{sddsplot character chart gallery}
provides 34 downloadable PNG charts covering all 32 named fonts and the original
Greek and special-symbol sets. Each glyph is labeled with the text needed in a
title or label. The gallery is included beside the HTML manual and in the
\verb|SDDSaps/doc/sddsplot-character-charts| source directory. When copying the
PDF manual, include this directory beside the PDF to preserve the gallery link.

Protect dollar signs from shell expansion. For example, in Bash:
\begin{verbatim}
sddsplot data.sdds -columnNames=x,y '-title=Angle $ga$r'
\end{verbatim}
Single quotes preserve the formatting codes. Inside double quotes, escape each
dollar sign, as in \verb|"-title=Angle \$ga\$r"|. An unescaped sequence such as
\verb|$y8$r| inside double quotes is interpreted as shell variables and can
disappear before reaching \verb|sddsplot|.

Use \verb|-font=name| to select the default font (\verb|rowmans| initially),
and \verb|-listFonts| to list fonts supported by the installed version.
Some punctuation uses the original symbol table regardless of the selected
font; the charts mark these glyphs. A literal tilde (\verb|~|) is not supported
by the character mapping, even when quoted correctly. \verb|$y3$r| draws an
arc, not a tilde.
...
\item {\tt c}, {\tt m}, {\tt k}, {\tt y}: select the \verb|cyrillic|,
\verb|mathlow|, \verb|greek|, and \verb|symbolic| fonts, respectively.
\verb|$r| restores the configured default font and exits Greek mode.

\item {\tt 1}, {\tt 2}, {\tt 3}: select \verb|rowmans|, \verb|rowmand|,
and \verb|rowmant|, respectively. For example, \verb|$2ABC$r| draws
\verb|ABC| with \verb|rowmand| and then restores the default font.
...
\end{itemize}
  ...
  \item \textbf{see also:}
    ...
    \item \hyperref[SDDSediting]{SDDS Editing}
    \item \hyperref[SDDS Wildcard Conventions]{SDDS Wildcard Conventions}
    \item \href{sddsplot-character-charts/index.html}{sddsplot character chart gallery}
    \end{itemize}
