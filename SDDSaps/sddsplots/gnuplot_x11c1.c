...
char color_keys[Ncolors][30] =   { "text", "line0", 
   "line1", "line2", "line3", "line4", "line5", "line6", "line7", "line8",
   "line9", "line10", "line11", "line12", "line13", "line14", "line15" 
       };
char color_values[Ncolors][30] = { "black", "white", 
   "red", "blue", "green", "yellow", "magenta", "cyan", "LimeGreen", "gold",
   "orange", "HotPink", "DeepSkyBlue", "MediumSpringGreen", "tomato", "tan", "grey75"
       } ;
...
int main(argc, argv) int argc; char *argv[]; {
   ...
   if (Color) {
	 ...
	 if (XAllocNamedColor(dpy, DefaultColormap(dpy,0), value, &used,&exact))
      ...
      for (n=0; n<Nspect; n++) {
          xcol.flags = 0xff;
          RGB_values(&xcol.red, &xcol.green, &xcol.blue, n/(Nspect-1.0));
          if (XAllocColor(dpy, DefaultColormap(dpy,0), &xcol)) {
              spectrum[n] = xcol.pixel;
              }
          else {
              fprintf(stderr, "gnuplot: cannot allocate hue=%f, RGB=%x:%x:%x\n", 
                      n/(Nspect-1.0), xcol.red, xcol.green, xcol.blue);
              spectrum[n] = BlackPixel(dpy,0);
              }
          }
      }
   XtSetArg(args[0], XtNwidth, &W);
...
int display() {
   ...
   for (n=0; n<nc; n++) {
	 ...
	 if (Color) {
             type = LineSolid;
            width = 1;
	    XSetForeground(dpy, gc, colors[lt]);
	    }
	 else {
	 ...
	 line_style = type;
      ...
      /*   X11_line_thickness(thickness) - set line width  */
      else if (*buf == 'W') { 
	 sscanf(buf, "W%4d", &lt);
	 if (!Color)
	   lt--;
	 XSetLineAttributes( dpy,gc, lt, line_style, CapButt, JoinBevel);
	 }
      /* Fill Box */
      else if (*buf=='B') {
          int shade, xl, xh, yl, yh;
          /* filled box */
          sscanf(buf, "B%04d%04d%04d%04d%04d", 
                 &shade, &xl, &xh, &yl, &yh);
          shade = shade%Nspect;
          xl = X(xl); yl = Y(yl);
          xh = X(xh); yh = Y(yh);
          XSetForeground(dpy, gc, spectrum[shade]);
          XFillRectangle(dpy, pixmap, gc, xl, yh, xh-xl, yl-yh);
          XSetForeground(dpy, gc, colors[lt+1]);
          }
...
void RGB_values(red, green, blue, hue)
unsigned short *red; unsigned short *green; unsigned short *blue; double hue;
{
