...
double	val;
{
   ...
   /* else */ if ( idx < -1 )
   {
	 ...
	 {
	   row_elt *tmp = r->elt ? realloc(r->elt, new_len * sizeof(row_elt)) : calloc(new_len, sizeof(row_elt));
	   r->elt = tmp;
	 }
...
SPMAT	*A, *OUT;
{
   ...
   if ( OUT->m < A->m )
   {
	 ...
	 mem_bytes(TYPE_SPMAT,A->max_m*sizeof(SPROW),
      ...
      {
	SPROW *tmp = OUT->row ? realloc(OUT->row, A->m * sizeof(SPROW)) : calloc(A->m, sizeof(SPROW));
	OUT->row = tmp;
      }
      if ( ! OUT->row )
...
int	m, n;
{
   ...
   else
   {
	 ...
	 mem_bytes(TYPE_SPMAT,A->max_m*sizeof(SPROW),
      ...
      {
	SPROW *tmp = A->row ? realloc(A->row, ((unsigned)m) * sizeof(SPROW)) : calloc((unsigned)m, sizeof(SPROW));
	A->row = tmp;
      }
      if ( ! A->row )
   ...
   {	/* only have to update the start_idx & start_row arrays */
      ...
      {
	int *tmp = A->start_row ? realloc(A->start_row, ((unsigned)n) * sizeof(int)) : calloc((unsigned)n, sizeof(int));
	A->start_row = tmp;
      }

      {
	int *tmp = A->start_idx ? realloc(A->start_idx, ((unsigned)n) * sizeof(int)) : calloc((unsigned)n, sizeof(int));
	A->start_idx = tmp;
      }
