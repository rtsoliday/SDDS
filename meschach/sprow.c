...
int	n,type;
{
   ...
   else
   {
	 ...
	 mem_bytes(type,r->maxlen*sizeof(row_elt),
      ...
      {
	row_elt *tmp = r->elt ? realloc(r->elt, newlen * sizeof(row_elt)) : calloc(newlen, sizeof(row_elt));
	r->elt = tmp;
      }
      if ( ! r->elt )
	error(E_MEM,"sprow_xpd");
...
int	n,type;
{
   ...
   else
   {
	 ...
	 mem_bytes(type,r->maxlen*sizeof(row_elt),
      ...
      {
	row_elt *tmp = r->elt ? realloc(r->elt, n * sizeof(row_elt)) : calloc(n, sizeof(row_elt));
	r->elt = tmp;
      }
      if ( ! r->elt )
	error(E_MEM,"sprow_resize");
