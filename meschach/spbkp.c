...
static int col_cmp(const void *a, const void *b)
{
   row_elt *e1 = (row_elt *)a;
   row_elt *e2 = (row_elt *)b;
...
SPMAT	*spBKPfactor(SPMAT *A, PERM *pivot, PERM *blocks, double tol)
{
	...
	if ( onebyone )
	{
	    int		idx_j, idx_k, s_idx=0, s_idx2;
	    row_elt	*e_ij, *e_ik;
    ...
    for ( i = 0; i < A->m; i++ )
	  qsort(A->row[i].elt,(size_t)(A->row[i].len),sizeof(row_elt), col_cmp);
